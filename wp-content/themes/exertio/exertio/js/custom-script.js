...
$(document).on('click', '.fl_addon_checkbox', function() {
	...
	var myclass = '.addon_price_'+addon_id+' .price';
	var addon_price = $(myclass).text();
	var service_price = $('.project-price.service .price .price').text();

	if($(this).prop('checked'))
	{
		setTimeout(function(){
			var total_price = parseInt(service_price) + parseInt(addon_price);
			$('.project-price.service .price .price').html(''+total_price.toFixed(2));
			$('#buy_service small span.price').html(''+total_price.toFixed(2));
			$('.loader-outer').hide();
		}, 300);
	} 
	else
