...
$("select").on("select2:select", function (evt) {
  ...
  $(this).trigger("change");
	...
	$( ".profile_selection" ).on( "click", function() {
		var active_profile = jQuery(this).attr("data-profile-active");
		$.cookie('active_profile', active_profile, { expires: 365, path: '/' });
...
/*DELET IMAGE FROM PROFILE*/	
	...
	$(document).on('click', '.add_new_skills',function(e) {
			  ...
			  $(".add_new_skills").prepend("<i class='fal fa-spinner-third fast-spin'></i>");
	...
	$(document).ready(function(){
				...
				var fieldHTML = '<div class="ui-state-default"><i class="fal fa-arrows"></i><div class="form-row"><input type="url" name="video_urls[]" value="" class="form-control"/></div><a href="javascript:void(0);" class="yt_url_remove"><i class="fal fa-times-circle"></i></a></div>'; 
				var x = 1; 
...
/*GENERAL FILE UPLOADER*/
	...
	$(document).on('click', '#service_rating_btn', function () {
		if( $('form#service_rating_form').smkValidate() ){
			...
			var status = $(this).attr('data-status');
		  $.post(localize_vars_frontend.freelanceAjaxurl, {action: 'fl_service_rating', rating_data: $("form#service_rating_form").serialize(), ongoing_sid: ongoing_sid,status: status, security:$('#gen_nonce').val() }).done(function (response)
			{
				if ( true === response.success ) 
				{
					this_value.find('div.bubbles').removeClass("view");
	...
	$(document).on('click', '#cancel-service-btn', function () {
		if( $('form#cancel-service-from').smkValidate() ){
			...
			var ongoing_sid = $(this).attr('data-ongoing-sid');
			var status = $(this).attr('data-status');
		  $.post(localize_vars_frontend.freelanceAjaxurl, {action: 'fl_service_rating', cancel_feedback: $("form#cancel-service-from").serialize(), ongoing_sid: ongoing_sid,status: status, security:$('#gen_nonce').val() }).done(function (response)
			{
				if ( true === response.success ) 
				{
					$(".btn-loading .bubbles").removeClass("view");
...
$(document).ready(function(){
				...
				$(document).on('click', '.add_faq_more', function (){
					...
					if(x < faqMaxFields){ 
						...
						var faqFieldHTML = '<div class="faqs-box"><h4>'+localize_vars_frontend.faqNo+' '+x+'</h4><ul><li><i class="fal fa-arrows"></i></li><li class="faq_more_remove"><i class="fal fa-times-circle"></i></li></ul><div class="faqs-box-meta"><div class="form-group"><input type="text" name="faqs-title[]" value="" class="form-control"></div><div class="form-group"><textarea name="faq-answer[]" id="" class="form-control"></textarea></div></div></div>'; 
...
$(document).on('click', '#deposit-funds-btn', function () {
	...
	$(document).on('click', '#despute-project-btn', function () {
		if( $('form#despute-project-form').smkValidate() ){
			var this_value = $(this);
			this_value.find('div.bubbles').addClass("view");
			$("#despute-project-btn").attr("disabled", true);
		  $.post(localize_vars_frontend.freelanceAjaxurl, {action: 'fl_despute_project_callback', despute_project_data: $("form#despute-project-form").serialize(), security:$('#fl_despute_nonce').val() }).done(function (response)
			{
				if ( true === response.success ) 
				{
					this_value.find('div.bubbles').removeClass("view");
				...
				{
					this_value.find('div.bubbles').removeClass("view");
					$("#despute-project-btn").attr("disabled", false);
					toastr.error(response.data.message, '', {timeOut: 8000, "closeButton": true, "positionClass": "toast-top-right", "showMethod": "slideDown", "hideMethod":"slideUp"});
				}
