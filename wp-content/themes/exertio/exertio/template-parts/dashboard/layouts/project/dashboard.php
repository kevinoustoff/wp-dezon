				...
				<div class="col-md-6 col-lg-6 col-xl-3">
					<div class="info-boxes">
						<div class="metric">
							...
							<p>
								<span class="title"><?php echo esc_html__('Projets sponsorisés', 'exertio_theme' ); ?></span>
				...
				<div class="col-md-6 col-lg-6 col-xl-3">
					<div class="info-boxes">
						<div class="metric">
							...
							<p>
								<span class="title"><?php echo esc_html__('Projets terminés', 'exertio_theme' ); ?></span>
								<span class="number"><?php echo esc_html($project_count_completed); ?></span>
							</p>
						</div>
						<p class="matric-bottom"> <a href="<?php echo esc_url(get_the_permalink());?>?ext=completed-projects"><?php echo esc_html__('Voir le détail', 'exertio_theme' ); ?> <i class="fas fa-arrow-right"></i></a></p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="col-xl-8 col-lg-12 col-md-12 stretch-card">
		  <div class="card  grid-margin">
			...
			<div class="card grid-margin all-proposals">
				<div class="card-body">
				  <h4 class="card-title"> <?php echo esc_html__('Offres récentes', 'exertio_theme' ); ?></h4>
						...
						<?php
							...
							if ( $result != null )
							{
								...
								foreach($result as $results)
								{
									...
									$posted_date = $results->updated_on;
											...
											{
												$project_cost .=  esc_html(fl_price_separator($results->proposed_cost));
												$project_cost .=  '<span class="protip" data-pt-title="'.__('En ','exertio_theme').$days_to_complete.__(' heures le total sera de  ','exertio_theme'). fl_price_separator($days_to_complete*$results->proposed_cost).'" data-pt-position="top" data-pt-scheme="black"><i class="far fa-question-circle"></i></span>';
											}
							...
							else
							{
								...
								<div class="nothing-found">
									...
									<h3><?php echo esc_html__( 'Désolé!! Aucune offre trouvée', 'exertio_theme' ) ?></h3>
								</div>
								<?php	
							}
						?>
									...
									while ( $most_listings->have_posts() )
									{
										...
										$project_cost = '';
											...
											else if($type == 'hourly')
											{
												...
												$project_cost .=  '<span class="protip" data-pt-title="'.__('En ','exertio_theme').$estimated_hours.__(' heures le total sera de  ','exertio_theme'). fl_price_separator($hourly_total_price).'" data-pt-position="top" data-pt-scheme="black"><i class="far fa-question-circle"></i></span>';
											}
									?>
											<li>
			...
			<div class="card grid-margin tips package-info current-package-widget">
				<div class="card-body">
				  <h4 class="card-title"><?php echo esc_html__('Abonnement actuel', 'exertio_theme' ); ?></h4>
					<p class="view-more-btn"> <a class="btn btn-primary" href="<?php echo get_the_permalink(fl_framework_get_options('emp_package_page')); ?>?ext=buy-package" target="_blank"> <?php echo esc_html__(' Voir les abonnements ', 'exertio_theme' ); ?></a></p>
					<?php
					...
					if(isset($simple_project) && $simple_project != '')
					{
						...
						<ul>
							...
							<li><img src="<?php echo get_template_directory_uri(); ?>/images/dashboard/success.png" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>"> <?php echo '<span>'.esc_html__('Projets sponsorisés: ', 'exertio_theme' ).'</span>'.esc_html($featured_projects_text);?> </li>
							<li><img src="<?php echo get_template_directory_uri(); ?>/images/dashboard/success.png" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>"> <?php echo '<span>'.esc_html__('Expiration du projet sponsorisé: ', 'exertio_theme' ).'</span>'.esc_html($featured_project_expiry_text); ?> </li>
							<li><img src="<?php echo get_template_directory_uri(); ?>/images/dashboard/success.png" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>"> <?php echo '<span>'.esc_html__('Profil sponsorisé: ', 'exertio_theme' ).'</span>'.esc_html($employer_featured_text); ?> </li>
							<li><img src="<?php echo get_template_directory_uri(); ?>/images/dashboard/success.png" alt="<?php echo esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', TRUE)); ?>"> <?php echo '<span>'.esc_html__('Expiration de l\'abonnement: ', 'exertio_theme' ).'</span>'.esc_html($package_expiry_text) ?> </li>
						</ul>
						<?php
					}
					else
					{
						echo '<p>'.esc_html__(' Aucun abonnement disponible pour le moment', 'exertio_theme' ).'</p>';
					}
					?>
			  </div>
			</div>
