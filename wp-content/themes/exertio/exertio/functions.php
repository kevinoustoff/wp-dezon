...
function exertio_scripts() {
	...
	function exertio_fonts_url() {
        $fonts_url = '';
        $source_sans = _x('on', 'Source Sans Pro: on or off', 'exertio_theme');
	...
	if ( is_singular( 'freelancer' ) || is_singular( 'services' ) || is_singular( 'projects' ) )
	{
	...
	wp_enqueue_script('select2', trailingslashit(get_template_directory_uri()) . 'js/select2.full.min.js', false, false, true);
	...
	wp_enqueue_style('web-font-icons', trailingslashit(get_template_directory_uri()) . 'css/font-all.css');
	...
	if ( is_page_template( 'page-services-search.php' ) || is_page_template( 'page-project-search.php' ) || is_page_template( 'page-freelancer-search.php' ) )
	{
	...
	if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
	{
		function inline_typography() 
		{
			...
			$custom_css = "
				...
				.btn-theme,  .post-excerpt .wp-block-button .wp-block-button__link, .post-excerpt .wp-block-search__button, .post-excerpt .wp-block-file .wp-block-file__button, .post-password-form input[type='submit'], .woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce #respond input#submit, .woocommerce a.button, .woocommerce button.button, .woocommerce input.button { border: 1px solid $main_btn_color; background-color: $main_btn_color; color: $main_btn_color_text; }
				.btn-theme:hover, .post-excerpt .wp-block-button .wp-block-button__link:hover, .post-excerpt .wp-block-search__button:hover, .post-excerpt .wp-block-file .wp-block-file__button:hover, .post-password-form input[type='submit']:hover, .woocommerce #respond input#submit.alt:hover, .woocommerce a.button.alt:hover, .woocommerce button.button.alt:hover, .woocommerce input.button.alt:hover, .woocommerce #respond input#submit:hover, .woocommerce a.button:hover, .woocommerce button.button:hover, .woocommerce input.button:hover {  background-color: $main_btn_color_hover; box-shadow: 0 0.5rem 1.125rem -0.5rem $main_btn_color_shadow; color: $main_btn_hover_color_text !important; border: 1px solid $main_btn_color_hover; }
				
				.btn-theme-secondary { border: 1px solid $sec_btn_color; background-color: $sec_btn_color; color: $sec_btn_color_text; }
				.btn-theme-secondary:hover { background-color: $sec_btn_color_hover; box-shadow: 0 0.5rem 1.125rem -0.5rem $sec_btn_color_shadow; color: $sec_btn_hover_color_text; border: 1px solid $sec_btn_color_hover; }
				...
				.sb-header .sb-menu li:not(:last-child) a:hover, .sb-header .sb-menu li:not(:last-child) a:focus, .sb-header .sb-menu li:not(:last-child) a:active, .fr-latest-content-service p a:hover, .fr-browse-category .fr-browse-content ul li a:hover, .fr-footer .fr-footer-content ul li a:hover, .fr-right-detail-box .fr-right-detail-content .fr-right-details2 h3:hover, .fr-right-detail-box .fr-right-detail-content .fr-right-details2 h3:hover, .blog-sidebar .widget ul li a:hover { color:$main_btn_color }
	...
	if(in_array('redux-framework/redux-framework.php', apply_filters('active_plugins', get_option('active_plugins'))))
	{
		...
		wp_localize_script('exertio-theme', 'localize_vars_frontend', array(
			...
			'AreYouSure' => __('En êtes vous sûr?','exertio_theme'),
			'Msgconfirm' => __('Confirmation','exertio_theme'),
			'remove' => __('Supprimer','exertio_theme'),
			'cancel' => __('Annuler','exertio_theme'),
			'AccDel' => __('Supprimer, Anyway','exertio_theme'),
			'proCancel' => __('Annuler, Anyway','exertio_theme'),
			'confimYes' => __('Oui','exertio_theme'),
			'confimNo' => __('Non','exertio_theme'),
			'awardDate' => esc_html__('Date de la récompense', 'exertio_theme'),
			'awardName' => esc_html__('Nom de la récompense', 'exertio_theme'),
			'selectImage' => esc_html__('Image', 'exertio_theme'),
			'projectURL' => esc_html__('Lien du projet', 'exertio_theme'),
			'projectName' => esc_html__('Nom du project', 'exertio_theme'),
			'expeName' => esc_html__('Poste occupée', 'exertio_theme'),
			'expeCompName' => esc_html__('Nom de l\'entreprise', 'exertio_theme'),
			'startDate' => esc_html__('Date de début', 'exertio_theme'),
			'endDate' => esc_html__('Date de fin', 'exertio_theme'),
			'endDatemsg' => esc_html__('Laissez le champ vide pour définir le poste actuel', 'exertio_theme'),
			'expeDesc' => esc_html__('Description', 'exertio_theme'),
			'eduName' => esc_html__('Titre de la formation ', 'exertio_theme'),
			'eduInstName' => esc_html__('Nom de l\'établissement', 'exertio_theme'),
			'eduEndDatemsg' => esc_html__('Laissez le champ vide pour définir votre formation actuelle', 'exertio_theme'),
			'proAdminCost' => $exertio_theme_options['project_charges'],
			'YesSure' => esc_html__('Oui, je suis sûr', 'exertio_theme'),
			'serviceBuy' => esc_html__('Etes vous sûr que vous voulez acheter ce service?', 'exertio_theme'),
			'maxFaqAllowed' => $exertio_theme_options['sevices_faqs_count'],
			'maxAllowedFields' => esc_html__('Nombre maximal de champs autorisé atteint', 'exertio_theme'),
			'faqNo' => esc_html__('FAQ N°', 'exertio_theme'),
			'is_reset' => $is_reset,
