...
if (!class_exists('exertio_get_services'))
{
	class exertio_get_services 
	{
		function exertio_listings_grid_1($service_id, $slider = '')
		{
			...
			{
				$featured_badge = '<div class="fr-top-featured"> <span class="badge">'.esc_html__( 'Sponsorisé', 'exertio_theme' ).'</span> </div>';
			}
		...
		function exertio_listings_grid_2($service_id, $slider = '')
		{
			...
			if(isset($featured_service) && $featured_service == 1)
			{
				$featured_badge = '<div class="fr-latest-btn"> <span class="badge">'.esc_html__( 'Sponsorisé', 'exertio_theme' ).'</span> </div>';
			}
			return '<div class="'.$cols.'  grid-item">
                        ...
                        <div class="col-xl-8 col-xs-12 col-lg-8 col-sm-8 col-md-8">
                        	<div class="exertio-services-box">
                                <div class="exertio-service-desc">
                                    ...
                                    <span class="desc-meta"> <?php echo __( 'par:', 'exertio_theme' ); ?> <a href="<?php echo esc_url(get_permalink($fid)); ?>"> <?php echo exertio_get_username('freelancer', $fid); ?></a></span>
                                    <div class="fr-top-rating"> <a href="javascript:void(0)" class="save_service protip <?php echo esc_attr($active_saved); ?>" data-fid="'.esc_html($fid).'" data-pt-position="top" data-pt-scheme="black" data-pt-title="<?php echo esc_html($save_text); ?>" data-post-id="<?php echo esc_attr($service_id); ?>"><i class="fa fa-heart"></i></a> </div>
                                </div>
                                <div class="exertio-services-bottom">
                                    <ul>
                                        <li> <?php echo esc_html__('A partir de  ','exertio_theme'); ?><span class="style-6"><?php echo fl_price_separator(get_post_meta($service_id, '_service_price', true), 'html'); ?></span></li>
