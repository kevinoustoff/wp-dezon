...
if (!class_exists('exertio_get_freelancers_class'))
{
	class exertio_get_freelancers_class 
	{
               ...
               <div class="<?php echo esc_attr($col_html); ?> grid-item">
				  <div class="fr3-product-detail-box">
                              ...
                              <?php
                                    ...
                                    foreach($saved_skills as $skills)
                                    {
										...
										if($skillsObject != '')
										{
											...
											{ $skill_hide = 'hide'; }
											?>
											<a href="<?php echo esc_url(get_term_link($skillsObject->term_id)); ?>"  class="<?php echo esc_html($skill_hide); ?>"><?php echo esc_html($skillsTermName); ?></a>
											<?php
										  $skill_count++;
										}
                                    }
                              ...
                              <?php
								...
								else
								{
									...
									if($hourly_rate != '')
									{
									  ?>
									  <li>
										  <span class="bottom-text"><?php echo esc_html__(' A partir de ','exertio_theme'); ?></span>
					  ...
					  <p>
						  ...
						  <?php echo esc_html__('Membre depuis ','exertio_theme').date_i18n( get_option( 'date_format' ), strtotime( get_the_date('dS /M/Y') ) ); ?></p>
					  </div>
					  
					  <div class="fr3-product-btn">
						  <a href="<?php echo get_the_permalink($freelancer_id); ?>" class="btn btn-theme"><?php echo esc_html__('Voir le profil','exertio_theme'); ?></a>
					  </div>
		...
		function exertio_freelancer_grid_2($freelancer_id, $cols = '') 
		{
			...
			if(isset($featured_freelancer) && $featured_freelancer == 1)
			{
				$featured_badge = '<div class="d-flex flex-row agent-type"><span class="badge badge-agent-type">'.esc_html__( 'Sponsorisé', 'exertio_theme' ).'</span></div>';
			}
			if($cols == '')
			{
                      ...
                      <div class="fr3-job-detail">
                          ...
                          <div class="fr3-job-text">
                              ...
                              <?php
								...
								?>
                                <ul class="lists">
                                    <li> <?php echo esc_html__('Membre depuis ','exertio_theme').date_i18n( get_option( 'date_format' ), strtotime( get_the_date('dS M Y') ) ); ?></li>
                                  </ul>
                                <?php
									$current_user_id = get_current_user_id();
									$saved_freelancer = get_user_meta($current_user_id, '_fl_follow_id_'.$freelancer_id, true);
									$active_saved ='';
									$save_text = esc_html__('Suivre','exertio_theme');
									if(isset($saved_freelancer) && $saved_freelancer != '')
									{
										$active_saved = 'active';
										$save_text = esc_html__('Suivi','exertio_theme');	
									}
								?>
                                <span class="follow <?php echo esc_attr($active_saved); ?> follow-freelancer protip" data-fid="<?php echo esc_html($freelancer_id); ?>" data-pt-position="top" data-pt-scheme="black" data-pt-title="<?php echo esc_html($save_text); ?>"> <i class="fas fa-heart"></i> <?php echo esc_html($save_text); ?></span>
                          </div>
                      </div>
                      <?php echo wp_return_echo($featured_badge);  ?>
                    </div>
                    ...
                    <div class="fr-lance-content3">
                      ...
                      <div class="fr-lance-more-details">
                        ...
                        <div class="fr-lance-products2">
                          ...
                          <p><i class="far fa-calendar-alt"></i><?php echo esc_html__('Member since ','exertio_theme').date_i18n( get_option( 'date_format' ), strtotime( get_the_date('dS M Y') ) ); ?></p>
                        </div>
                        <?php
					  $saved_skills =  json_decode(stripslashes(get_post_meta($freelancer_id, '_freelancer_skills', true)), true);
					  if($saved_skills != '')
