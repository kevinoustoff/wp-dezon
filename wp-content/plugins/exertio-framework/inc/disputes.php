...
if ( ! function_exists( 'fl_get_all_project_services' ) )
{
	function fl_get_all_project_services()
	{
		...
		$html ='';
		$html .= '<select name="project_id" class="general_select form-control" required data-smk-msg="'.esc_html__('Select select project first','exertio_theme').'"><option value="">'.esc_html__("Select project","exertio_framework").'</option>';
...
/*DESPUTE CREATE CALLBACK*/
add_action('wp_ajax_fl_despute_project_callback', 'fl_despute_project_callback');
add_action('wp_ajax_nopriv_fl_despute_project_callback', 'fl_despute_project_callback');
if ( ! function_exists( 'fl_despute_project_callback' ) )
{ 
	function fl_despute_project_callback()
	{
		exertio_demo_disable('json');
		check_ajax_referer( 'fl_despute_secure', 'security' );
		$current_user_id = get_current_user_id();
		parse_str($_POST['despute_project_data'], $params);
	...
	function exertio_dispute_msg()
	{
		...
		if($params['dispute_msg_text'] != '')
		{
			...
			$data = array(
						...
						'dispute_id' => sanitize_text_field($pid),
