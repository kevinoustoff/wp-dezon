	...
	function exertio_employer_order_status_completed( $order_id )
	{
		...
		foreach ( $items as $item )
		{
			...
			if($prduct_type == 'employer-packages') 
			{
				...
				$ext_simple_projects = get_post_meta($product_id, '_simple_projects', true);
				if($ext_simple_projects == -1)
				{
					$simple_projects = $ext_simple_projects;
				}
				...
				$ext_simple_project_expiry = get_post_meta($product_id, '_simple_project_expiry', true);
				if($ext_simple_project_expiry == -1)
				{
					$simple_project_expiry = $ext_simple_project_expiry;
				}
				...
				$ext_featured_projects = get_post_meta($product_id, '_featured_projects', true);
				if($ext_featured_projects == -1)
				{
					$featured_projects = $ext_featured_projects;
				}
				...
				$ext_featured_project_expiry = get_post_meta($product_id, '_featured_project_expiry', true);
				if($ext_featured_project_expiry == -1)
				{
					$featured_project_expiry = $ext_featured_project_expiry;
				}
				...
				$ext_employer_package_expiry = get_post_meta($product_id, '_employer_package_expiry', true);
				if($ext_employer_package_expiry == -1)
				{
					$employer_package_expiry = $ext_employer_package_expiry;
				}
				...
				$ext_employer_is_featured = get_post_meta($product_id, '_employer_is_featured', true);
