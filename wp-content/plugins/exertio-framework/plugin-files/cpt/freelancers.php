...
function custom_freelancer_column( $column, $post_id ) {
	...
	$departments_labels = array(
		...
		register_taxonomy('number-of-employees', array('companies'), array(
			'hierarchical' => false,
			'show_ui' => true,
			...
			'query_var' => true,
			'meta_box_cb' => false,
			'rewrite' => array('slug' => 'number-of-employees'),
		));		
	  ...
	  <?php wp_nonce_field( basename( __FILE__ ), 'freelancer_post_class_nonce' ); 
        ...
        <div class="custom-row">
            ...
            <select name="freelancer_gender">
            	<option value="male"><?php echo __( "Male", 'exertio_framework' ); ?> </option>
                <option value="female"><?php echo __( "Female", 'exertio_framework' ); ?> </option>
            </select>
            </div>
        </div>
        <div class="custom-row">
        ...
        <div class="custom-row">
		  ...
		  $location_taxonomies = exertio_get_terms('freelancer-locations');
			if ( !empty($location_taxonomies) )
			{
				...
				echo '<select name="freelancer_location" class="form-control general_select">'.get_hierarchical_terms('freelancer-locations', $fl_location ).'</select>';
			}
            else
            {
                echo __( "No values available. Please consider adding values first", 'exertio_framework' );
                    ...
                    <ul class="freelance_gallery">
                        ...
                        ?>
                                <?php
											...
											echo '<li id="data-'.$profile_pic_attachment_ids.'"><img src="'.$attachment_data.'" alt="'.__( "attachment", 'exertio_framework' ).'"></li>';
                                ?>
                    ...
                    <ul class="freelance_banner_gallery">
                        ...
                        if(isset($banner_attachment_id) && $banner_attachment_id !='')
                        {
                                        $banner_attachment_data = wp_get_attachment_url( $banner_attachment_id );
											echo '<li id="data-'.$banner_attachment_id.'"><img src="'.$banner_attachment_data.'" alt="'.__( "attachment", 'exertio_framework' ).'"></li>';
                        }
                        ?> 
                    </ul>
                 </div> 
            ...
            <div class="col-9">
                <div class="col-12">
                	<div class="skills_wrapper sortable"  id="sortable">
                    	<?php
							$skills_json =  json_decode(get_post_meta($post_id, '_freelancer_skills', true), true);
                            ...
                            <div class="ui-state-default">
                                ...
                                <div class="col-4">
                                    ...
                                    {
                                        $skills = '<select name="freelancer_skills[]">';
                                        foreach( $skills_taxonomies as $skills_taxonomy ) {
                                            if( $skills_taxonomy->parent == 0 ) {
                                                 $skills .= '<option value="'. esc_attr( $skills_taxonomy->term_id ) .'">
                                                        '. esc_html( $skills_taxonomy->name ) .'</option>';
                                                $skills.='</option>';
                                            }
                                        }
                                ...
                                <div class="col-4">
                                    <input type="number" name="skills_percent[]" min="1" max="100" placeholder="<?php echo __( "Skills percentage", 'exertio_framework' ); ?>">
                                </div>
                            </div>
                            <?php
            ...
            <div class="col-9">
                <div class="col-12">
                	<div class="award_wrapper sortable" id="sortable">
                    	<?php
							$award_jsons =  json_decode(get_post_meta($post_id, '_freelancer_awards', true), true);
							if(!empty($award_jsons))
							{
								/*for($i=0; $i<count($award_json); $i++)
								{
									echo $i;
									echo '<br>';
									$award_img_url = wp_get_attachment_url( $award_json[$i]['award_img'] );
									echo $award_img_url;
									$award_html .= '<div class="ui-state-default" id="award_'.$i.'"><span class="dashicons dashicons-move"></span><div class="col-4"><input type="text" name="award_name[]" value="'.$award_json[$i]['award_name'].'" ></div><div class="col-4"> <input type="text" class="datetimepicker" name="award_date[]" value="'.$award_json[$i]['award_date'].'"></div> <div class="col-4"> <button type="button" class="award_img_btn_'.$i.' button button-primary button-large">'.__( "Select image", 'exertio_framework' ).'</button> <input type="hidden" id="award_img_id_'.$i.'" name="award_img_id[]" value="'.$award_json[$i]['award_img'].'"> <div class="award_banner_gallery sort_imgs"></div> </div><a href="javascript:void(0);" class="remove_button"><img src="'.FL_PLUGIN_URL.'/images/error.png" ></a></div>';
            ...
            <div class="col-9">
                <div class="col-12">
                	<div class="project_wrapper sortable" id="sortable">
                    	<?php
							$project_jsons =  json_decode(get_post_meta($post_id, '_freelancer_projects', true), true);
            ...
            <div class="col-9">
                <div class="col-12">
                	<div class="edu_wrapper sortable" id="sortable">
                    	<?php
							$edu_jsons =  json_decode(get_post_meta($post_id, '_freelancer_education', true), true);
							if(!empty($edu_jsons))
							{
								$edu_html = '';
								$count = '1';
       ...
       if(isset($_POST['freelance_type']))
		...
		if(isset($_POST['freelancer_skills']))
		{
			...
			for($i=0; $i<count($skill_name); $i++)
			{
			 ...
			 $encoded_skills =  json_encode($skills);
		...
		if(isset($_POST['award_name']) && isset($_POST['award_date']))
		{
			...
			$encoded_awards =  json_encode($awards);
			//echo $encoded_awards;
			//exit;
		...
		if(isset($_POST['project_name']) && isset($_POST['project_url']))
		{
			...
			$encoded_projects =  json_encode($projects);
