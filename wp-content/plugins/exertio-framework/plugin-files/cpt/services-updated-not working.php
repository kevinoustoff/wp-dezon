...
function fl_services_themes_custom_types() {
	...
	function services_post_class_meta_box( $post ) { ?>
	  ...
	  <?php wp_nonce_field( basename( __FILE__ ), 'service_post_class_nonce' ); 
        ...
        <div class="custom-row">
            ...
            {
				$service_category = get_post_meta($post_id, '_service_category', true);
				$category = '<select name="service_category">';
				$category .= '<option value=""> '. __( "Select Category", "exertio_framework" ) .'</option>';
                foreach( $categories_taxonomies as $categories_taxonomy ) {
					if($categories_taxonomy->term_id == $service_category){ $selected = 'selected ="selected"';}else{$selected = ''; }
                    if( $categories_taxonomy->parent == 0 ) {
                         $category .= '<option value="'. esc_html( $categories_taxonomy->term_id ) .'" '.$selected.'>
                                '. esc_html( $categories_taxonomy->name ) .'</option>';
                        $category.='</option>';
                    }
                }
                $category.='</select>';
        ...
        <div class="custom-row">
            ...
            {
				$service_location = get_post_meta($post_id, '_service_location', true);
                $pro_location = '<select name="service_location">';
				$pro_location .= '<option value=""> '. __( "Select location", "exertio_framework" ) .'</option>';
                foreach( $location_taxonomies as $location_taxonomy ) {
					if($location_taxonomy->term_id == $service_location){ $selected = 'selected ="selected"';}else{$selected = ''; }
                    if( $location_taxonomy->parent == 0 ) {
                         $pro_location .= '<option value="'. esc_attr( $location_taxonomy->term_id ) .'" '.$selected.'>
                                '. esc_html( $location_taxonomy->name ) .'</option>';
                        $pro_location.='</option>';
                    }
                }
                $pro_location.='</select>';
	...
	function services_save_post_class_meta( $post_id, $post ) {
	  ...
	  if ( !current_user_can( $post_type->cap->edit_post, $post_id ) )
		...
		{
			$category_terms = array((int)$_POST['service_category']); 
			/*print_r($cost_terms);
			exit;*/
			update_post_meta( $post_id, '_service_category', $_POST['service_category']);
			wp_set_post_terms( $post_id, $category_terms, 'service-categories', false );
       ...
       if(isset($_POST['response_time']))
		...
		{
			$service_location_term = array((int)$_POST['service_location']); 
			update_post_meta( $post_id, '_service_location', $_POST['service_location']);
			wp_set_post_terms( $post_id, $service_location_term, 'services-locations', false );
