...
function fl_themes_custom_types() {
            ...
            <div class="col-3">
              ...
              $category_taxonomies = exertio_get_terms('project-categories');
                if ( !empty($category_taxonomies) )
                {
                    ...
                    echo '<select name="project_category" class="form-control general_select">'.get_hierarchical_terms('project-categories', '_project_category', $post_id ).'</select>';
                }
                else
                {
        ...
        <div class="custom-row">
            ...
            <div class="col-3">
                <select name="project_type" class="form-control general_select project-type" required >
                    <option value=""> <?php echo esc_html__( "Select Project Type", "exertio_theme" ); ?></option>
                    <option value="hourly" <?php if($project_type== 'hourly') {echo 'selected="selected"';} ?>><?php echo esc_html__('Hourly','exertio_theme'); ?></option>
                    <option value="fixed" <?php if($project_type== 'fixed') {echo 'selected="selected"';} ?>><?php echo esc_html__('Fixed','exertio_theme'); ?></option>
                </select>
            </div>
        </div>
	...
	function project_save_post_class_meta( $post_id, $post ) {
	  ...
	  if ( !current_user_can( $post_type->cap->edit_post, $post_id ) )
		...
		{
			/*$category_terms = array((int)$_POST['project_category']); 
			update_post_meta( $post_id, '_project_category', $_POST['project_category']);
			wp_set_post_terms( $post_id, $category_terms, 'project-categories', false );*/
       ...
       if(isset($_POST['project_level']))
		...
		else if($_POST['project_type'] == 'hourly')
		{
		...
		{
			/*$project_location_term = array((int)$_POST['project_location']); 
			update_post_meta( $post_id, '_project_location', $_POST['project_location']);
			wp_set_post_terms( $post_id, $project_location_term, 'locations', false );*/
