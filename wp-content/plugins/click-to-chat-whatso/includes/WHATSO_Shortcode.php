...
class WHATSO_Shortcode {
	...
	public function __construct () {
		...
		add_action( 'wp_footer', array( 'WHATSO_Shortcode', 'addStyles' ) );
		
	}
	
	public function whatsappButton ( $atts ) {
		...
		foreach ( $account as $post ) {
			setup_postdata( $post );
			$item.= '<p id="whatso-button-' . $post->ID . '" class="whatso-button-container" data-target-id="' . $post->ID . '" data-page-title="' . $page_title . '" data-page-url="' . $page_url . '"><span></span></p>';
		 }
	...
	public function shortcodeWhatsApp ( $atts, $content = null ) {
		...
		if ( '' !== $text_color ||
			...
			$fa = '<svg class="WhatsApp" width="15px" height="15px" viewBox="0 0 90 90"><use xlink:href="#whatso-logo"></svg>';
